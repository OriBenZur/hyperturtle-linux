...
#define HYPERUPCALL_N_MAP_SLOTS 4
#define HYPERUPCALL_PROG_NAME_LEN 1024
#define HYPERUPCALL_MAX_N_MEMSLOTS 16

#define HYPERUPCALL_MAJORID_XDP 0
#define HYPERUPCALL_MAJORID_PAGEFAULT 1
#define HYPERUPCALL_MAJORID_MAX 2
...
static const char * const memory_backend_ids[HYPERUPCALL_N_MAP_SLOTS] = {"hp0", "hp1", "hp2", "hp3"};
static const char * const memory_backend_names[HYPERUPCALL_N_MAP_SLOTS] = {"bpf_map_obj0", "bpf_map_obj1", "bpf_map_obj2", "bpf_map_obj3"};
static const char * const memory_devices_names[HYPERUPCALL_N_MAP_SLOTS] = {"bpf_map_dev0", "bpf_map_dev1", "bpf_map_dev2", "bpf_map_dev3"};
static int used_memslots;
static unsigned long long memslot_base_gfns_local[HYPERUPCALL_N_MAP_SLOTS];
static unsigned long long memslot_npages_local[HYPERUPCALL_N_MAP_SLOTS];
static unsigned long long memslot_userptrs_local[HYPERUPCALL_N_MAP_SLOTS];
...
err:
    ...
    if (used_memslots < HYPERUPCALL_MAX_N_MEMSLOTS) {
        memslot_base_gfns_local[used_memslots] = (unsigned long long)mem.guest_phys_addr;
        memslot_npages_local[used_memslots] = (unsigned long long)mem.memory_size;
        memslot_userptrs_local[used_memslots] = (unsigned long long)mem.userspace_addr;
        used_memslots++;
    }
    else {
        fprintf(stderr, "No more memslots available\n");
    }
    return ret;
}
...
static int map_hyperupcall_map(CPUState *cpu, MemTxAttrs attrs, unsigned int hyperupcall_slot, char *map_name_guest) {
    ...
    sprintf(mmaped_map_size, "%d", bpf_map__max_entries(map)*bpf_map__value_size(map));
